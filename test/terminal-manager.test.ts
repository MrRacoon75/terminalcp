...
describe("TerminalManager", () => {
	...
	it("should handle stream output with since_last", async () => {
		...
		const stream2 = await manager.getStream(id, { since_last: true });
	...
	it("should stop all processes", async () => {
		...
		const id1 = await manager.start("sleep 10", { name: "sleep1" });
		const id2 = await manager.start("sleep 10", { name: "sleep2" });
		const id3 = await manager.start("sleep 10", { name: "sleep3" });
